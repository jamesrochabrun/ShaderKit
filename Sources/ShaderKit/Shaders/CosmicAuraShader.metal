...
[[stitchable]] half4 cosmicAura(
    ...
    if (fullImageOverlay) {
        float edgeSoftness = smoothstep(0.0, 0.11, edgeDistance);
        float centerRelief = smoothstep(0.15, 0.42, radius);
        ringMask = edgeSoftness * (0.22 + centerRelief * 0.78);
        normalizedRing = clamp(radius / max(auraRadius, 0.50), 0.0, 1.0);
    } else {
    ...
    half3 rgb = color * half(glow * 1.22) + bloomColor * half(edgeBloom * 0.18);
    float alpha = clamp((glow * 0.70 + particles * 0.45) * intensity, 0.0, 0.92);

    float centerClear = fullImageOverlay ? (0.38 + smoothstep(0.16, 0.44, radius) * 0.62) : smoothstep(inner + 0.020, inner + 0.055, radius);
    alpha *= centerClear;
